...
class ItemsController {
  async createItem(data: CreateItemBody, ctx: ValidationContext | null) {
    if (!ctx) {
  ...
  async get(request: NextRequest) {
    ...
    try {
      ...
      const baseQuery = queryBuilder.orderBy("postedAt", "desc")
      ...
      if (searchTerms.length === 0) {
        ...
        try {
          ...
          try {
            ...
            log.warn("Items list used fallback query without orderBy", {
              userId: decoded.uid,
              postedBy: query.postedBy ?? null,
        ...
        let itemsAfterBlock = items
        const userRef = usersCollection().doc(decoded.uid)
        const userSnap = await userRef.get()
        const blockedUserIds = (userSnap.data()?.blockedUserIds as string[] | undefined) ?? []
        if (blockedUserIds.length > 0) {
          const blockedSet = new Set(blockedUserIds)
          itemsAfterBlock = items.filter((it) => !blockedSet.has(it.postedBy))
        }
        hasMore = itemsAfterBlock.length > query.pageSize
        page = itemsAfterBlock.slice(0, query.pageSize)

        if (page.length === 0 && process.env.NODE_ENV === "production") {
          console.warn(
            `[Items API] No items found. Query filters: status=${query.status || "all"}, categories=${query.categories?.join(",") || "none"}, postedBy=${query.postedBy || "none"}, snapshot.size=${snapshot.size}, parsed items=${items.length}`
          )
          try {
        ...
        try {
          const searchQuery = baseQuery.where("searchKeywords", "array-contains-any", searchTermsForQuery)
          ...
          let itemsAfterBlock = filteredItems
          const userRef = usersCollection().doc(decoded.uid)
          const userSnap = await userRef.get()
          const blockedUserIds = (userSnap.data()?.blockedUserIds as string[] | undefined) ?? []
          if (blockedUserIds.length > 0) {
            const blockedSet = new Set(blockedUserIds)
            itemsAfterBlock = filteredItems.filter((it) => !blockedSet.has(it.postedBy))
          }
          hasMore = itemsAfterBlock.length > query.pageSize || hasNextBatch
          page = itemsAfterBlock.slice(0, query.pageSize)
        } catch (searchError) {
          log.warn("Items search used in-memory fallback after index query failure", {
            userId: decoded.uid,
            searchTerms,
            pageSize: query.pageSize,
      ...
      const shouldCheckFavorites =
        query.includeFavoriteStatus !== false && page.length > 0 && query.postedBy !== decoded.uid
...
export const POST = withValidation(
  ...
  async (_req, data: CreateItemBody, ctx: ValidationContext | null) => {
    return controller.createItem(data, ctx)
  },
