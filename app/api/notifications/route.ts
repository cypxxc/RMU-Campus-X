...
class NotificationsController {
  async get(request: NextRequest) {
    try {
      ...
      const pageSize =
        Number.isFinite(pageSizeRaw) && pageSizeRaw > 0 ? Math.min(pageSizeRaw, 50) : 20
      ...
      try {
        ...
        includeTotalCount =
          parseBooleanParam(searchParams.get("includeTotalCount"), "includeTotalCount") ??
          true
      } catch (parseError) {
        const message =
          parseError instanceof Error ? parseError.message : "Invalid query parameters"
  ...
  async post(request: NextRequest) {
    try {
      ...
      const effectiveSenderId = adminOverride ? requestedSenderId ?? requesterUserId : requesterUserId
