...
class ChatMessageController {
  async delete(request: NextRequest, params: Promise<{ exchangeId: string; messageId: string }>) {
    try {
      ...
      if (!exchangeId || !messageId) {
        return ApiErrors.badRequest("Missing exchangeId or messageId")
      }
      ...
      const ownerId =
        exchangeData && typeof exchangeData.ownerId === "string" ? exchangeData.ownerId : ""
      const requesterId =
        exchangeData && typeof exchangeData.requesterId === "string" ? exchangeData.requesterId : ""
      ...
      if (!isParticipantUser && !admin) {
        return ApiErrors.forbidden("เฉพาะผู้เกี่ยวข้องหรือแอดมินเท่านั้น")
      }
      if (isParticipantUser && !admin && (await isExchangeBlocked(db, ownerId, requesterId))) {
      ...
      if (senderId !== decoded.uid && !admin) {
        return ApiErrors.forbidden("ลบได้เฉพาะข้อความของตัวเอง")
      }
  ...
  async patch(request: NextRequest, params: Promise<{ exchangeId: string; messageId: string }>) {
    try {
      const token = getAuthToken(request)
      if (!token) return ApiErrors.unauthorized("Missing authentication token")
      const decoded = await verifyIdToken(token, true)
      ...
      if (senderId !== decoded.uid && !admin) {
        return ApiErrors.forbidden("แก้ไขได้เฉพาะข้อความของตัวเอง")
      }
...
export async function DELETE(
  ...
  return controller.delete(_request, params)
}
