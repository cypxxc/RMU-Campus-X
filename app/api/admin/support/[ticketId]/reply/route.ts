...
class AdminSupportTicketReplyController {
  async post(request: NextRequest, params: Promise<{ ticketId: string }>) {
    ...
    try {
      ...
      const replyPreview = replyText.length > 300 ? `${replyText.slice(0, 300).trim()}...` : replyText
...
const controller = new AdminSupportTicketReplyController()
