...
class AdminReportNotifyOwnerController {
  async post(request: NextRequest, params: Promise<{ id: string }>) {
    ...
    try {
      ...
      try {
        ...
        if (userSnap.exists) {
          ...
          if (lineUserId && lineEnabled) {
            await notifyUserReported(lineUserId, reportType, targetTitle, reportReason || undefined)
          }
        }
      } catch (lineErr) {
      ...
      return errorResponse(AdminErrorCode.INTERNAL_ERROR, "ไม่สามารถแจ้งเตือนได้", 500)
    }
  }
}

const controller = new AdminReportNotifyOwnerController()
