...
class ExchangeController {
      ...
      if (targetUserDoc.exists) {
        ...
        if (targetUserData.lineUserId && targetUserData.lineNotifications?.enabled) {
          if (status === "completed" && targetUserData.lineNotifications.exchangeComplete) {
            await notifyExchangeCompleted(targetUserData.lineUserId, exchangeData.itemTitle)
  ...
  async get(request: NextRequest, params: Promise<{ id: string }>) {
    try {
      ...
      if (exchangeData.cancelledAt) {
        out.cancelledAt = ts((exchangeData as { cancelledAt?: unknown }).cancelledAt)
      }
