...
class ReviewsService {
  ...
  async getUserReviews(userId: string, limitCount = 10): Promise<Review[]> {
  ...
  checkExchangeReviewed = async (exchangeId: string, reviewerId: string): Promise<boolean> => {
    const db = getFirebaseDb()
    const q = query(
      collection(db, "reviews"),
      where("exchangeId", "==", exchangeId),
...
export const getUserReviews = async (userId: string, limitCount = 10): Promise<Review[]> =>
  reviewsService.getUserReviews(userId, limitCount)
...
export const checkExchangeReviewed = async (exchangeId: string, reviewerId: string): Promise<boolean> =>
  reviewsService.checkExchangeReviewed(exchangeId, reviewerId)
