...
export class CloudinaryUtilsService {
  extractCloudinaryPublicId(imageRef: unknown): string | null {
  ...
  extractItemImagePublicIds(itemData: Record<string, unknown>): string[] {
...
const cloudinaryUtilsService = new CloudinaryUtilsService()

export function extractCloudinaryPublicId(imageRef: unknown): string | null {
  return cloudinaryUtilsService.extractCloudinaryPublicId(imageRef)
}

export function extractItemImagePublicIds(itemData: Record<string, unknown>): string[] {
  return cloudinaryUtilsService.extractItemImagePublicIds(itemData)
}
