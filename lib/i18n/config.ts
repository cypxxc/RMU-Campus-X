...
class LocaleConfigService {
  ...
  parseLocale(value: string | null | undefined): Locale | null {
    if (!value) return null
    const baseLocale = value.trim().toLowerCase().split("-")[0]
    return baseLocale && this.isLocale(baseLocale) ? baseLocale : null
  }

  resolveLocale(value: string | null | undefined): Locale {
    return this.parseLocale(value) ?? DEFAULT_LOCALE
  }

  localeFromAcceptLanguage(headerValue: string | null | undefined): Locale {
...
const localeConfigService = new LocaleConfigService()

export function isLocale(value: string): value is Locale {
  return localeConfigService.isLocale(value)
}

export function parseLocale(value: string | null | undefined): Locale | null {
  return localeConfigService.parseLocale(value)
}

export function resolveLocale(value: string | null | undefined): Locale {
  return localeConfigService.resolveLocale(value)
}

export function localeFromAcceptLanguage(headerValue: string | null | undefined): Locale {
  return localeConfigService.localeFromAcceptLanguage(headerValue)
}
