...
class LineCoreService {
  private withDefaultReplyQuickReply(messages: LineMessage[]): LineMessage[] {
  ...
  getDefaultRichMenuId(): string | null {
    const richMenuId = process.env.LINE_RICH_MENU_DEFAULT_ID?.trim()
    return richMenuId ? richMenuId : null
  }

  private extractLineApiErrorMessage(status: number, text: string, payload: unknown): string {
    if (payload && typeof payload === "object") {
      ...
      const baseMessage =
        typeof obj.message === "string" && obj.message.trim() ? obj.message.trim() : `HTTP ${status}`
  ...
  private async lineApiRequest<T = unknown>(
    path: string,
    init: {
      method: string
      headers?: HeadersInit
    ...
    try {
      ...
      if (!response.ok) {
        return {
          ...
          error: this.extractLineApiErrorMessage(response.status, text, payload),
        }
      }
 ...
 */
  async sendPushMessage(lineUserId: string, messages: LineMessage[]): Promise<LinePushResponse> {
    const result = await this.lineApiRequest("/message/push", {
      method: "POST",
      body: {
        to: lineUserId,
 ...
 */
  async sendReplyMessage(replyToken: string, messages: LineMessage[]): Promise<LinePushResponse> {
    const result = await this.lineApiRequest("/message/reply", {
      method: "POST",
      body: {
        replyToken,
        messages: this.withDefaultReplyQuickReply(messages).slice(0, 5),
      },
    })
    if (!result.success) {
 ...
 */
// ============ Rich Menu Utilities ============
  async listRichMenus(): Promise<LineRichMenuListResponse> {
    const result = await this.lineApiRequest<{ richmenus?: LineRichMenu[] }>("/richmenu/list", {
      method: "GET",
    })
    if (!result.success) {
    ...
    const richMenus = Array.isArray(result.data?.richmenus) ? result.data.richmenus : []
    return { success: true, richMenus }
  }

  async getDefaultRichMenu(): Promise<LineRichMenuIdResponse> {
    const result = await this.lineApiRequest<{ richMenuId?: string }>("/user/all/richmenu", {
      method: "GET",
    })
    if (!result.success) {
  ...
  async setDefaultRichMenu(richMenuId: string): Promise<LinePushResponse> {
    const result = await this.lineApiRequest(`/user/all/richmenu/${encodeURIComponent(richMenuId)}`, {
      method: "POST",
    })
    if (!result.success) {
  ...
  async clearDefaultRichMenu(): Promise<LinePushResponse> {
    const result = await this.lineApiRequest("/user/all/richmenu", { method: "DELETE" })
    if (!result.success) {
  ...
  async getUserRichMenu(lineUserId: string): Promise<LineRichMenuIdResponse> {
    const result = await this.lineApiRequest<{ richMenuId?: string }>(
  ...
  async linkRichMenuToUser(lineUserId: string, richMenuId: string): Promise<LinePushResponse> {
    const result = await this.lineApiRequest(
  ...
  async removeRichMenuFromUser(lineUserId: string): Promise<LinePushResponse> {
    const result = await this.lineApiRequest(`/user/${encodeURIComponent(lineUserId)}/richmenu`, {
      method: "DELETE",
    })
    if (!result.success) {
      if (result.status === 404) return { success: true }
  ...
  async applyDefaultRichMenuToUser(lineUserId: string): Promise<LineRichMenuApplyResponse> {
    const richMenuId = this.getDefaultRichMenuId()
    if (!richMenuId) {
      return { success: true, skipped: true, richMenuId: null }
    }
    const linked = await this.linkRichMenuToUser(lineUserId, richMenuId)
    if (!linked.success) {
 ...
 */
  async verifySignature(body: string, signature: string): Promise<boolean> {
    try {
      ...
      console.log(
        "[LINE] verifySignature: Verifying with secret (first 5 chars):",
        channelSecret.substring(0, 5) + "..."
      )
...
const lineCoreService = new LineCoreService()

export function getChannelAccessToken(): string | null {
  return lineCoreService.getChannelAccessToken()
}

export function getChannelSecret(): string | null {
  return lineCoreService.getChannelSecret()
}

export function getDefaultRichMenuId(): string | null {
  return lineCoreService.getDefaultRichMenuId()
}
...
export async function sendPushMessage(
  ...
  return lineCoreService.sendPushMessage(lineUserId, messages)
}
