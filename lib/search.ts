...
class SearchService {
  ...
  similarityScore(query: string, text: string): number {
  ...
  searchItems<T extends Record<string, unknown>>(
    items: T[],
    query: string,
    ...
    for (const item of items) {
      ...
      for (const field of searchFields) {
        ...
        const score = this.similarityScore(query, value)
        if (score >= minScore) {
  ...
  generateSuggestions(
    ...
    const suggestions = availableTerms
      .map(term => ({
        term,
        score: this.similarityScore(query, term),
      }))
...
export function searchItems<T extends Record<string, unknown>>(
  ...
  return searchService.searchItems(items, query, searchFields, options)
}
...
export function highlightMatches(
  ...
  return searchService.highlightMatches(text, query, highlightClass)
}
...
export function generateSuggestions(
  ...
  return searchService.generateSuggestions(query, availableTerms, maxSuggestions)
}
...
export class SearchIndex<T extends Record<string, unknown>> {
  ...
  index(items: T[]): void {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (const field of this.searchFields) {
        ...
        const tokens = searchService.tokenize(value)
  ...
  search(query: string, maxResults = 50): T[] {
    const queryTokens = searchService.tokenize(query)
